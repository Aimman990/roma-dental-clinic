...
@section('header', 'إنشاء خطة علاج جديدة')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 md:p-8">
        <form action="{{ route('treatment-plans.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <div class="relative" x-data="{ search: '', show: false, selected: null, patients: {{ $patients->map(fn($p)=>['id'=>$p->id, 'name'=>$p->name, 'phone'=>$p->phone])->toJson() }} }">
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" class="w-full border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" rows="2"></textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <select name="doctor_id" class="w-full border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                    <option value="">(اختيار) الطبيب افتراضياً سيكون المسجل الحالي</option>
                ...
                <template x-for="(row, index) in rows" :key="index">
                    <div class="bg-slate-50 p-4 rounded-lg border border-slate-200 relative grid grid-cols-1 md:grid-cols-12 gap-4">
                        ...
                        <div class="md:col-span-4">
                            ...
                            <!-- Hidden service ID if we want to implement select later -->
            ...
            <div class="flex items-center justify-end gap-4 pt-6 mt-6 border-t border-slate-100">
                ...
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">حفظ الخطة</button>
            </div>
        </form>
    </div>
</div>
